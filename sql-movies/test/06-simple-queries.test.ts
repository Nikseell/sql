...
describe("Simple Queries", () => {
  ...
  it(
    ...
    async done => {
      const query = `SELECT substr(release_date, 1, 4) as year, COUNT(*) as count FROM movies GROUP BY year ORDER BY year DESC`;
      const result = await db.selectMultipleRows(query);
  ...
  it(
    ...
    async done => {
      const query = `SELECT substr(time_created, 6, 2) as month, COUNT(*) as count from movie_ratings GROUP BY month ORDER BY count DESC`;
      const result = await db.selectMultipleRows(query);
